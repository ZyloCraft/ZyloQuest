...
import { Options, Client as userClient } from "discord.js-selfbot-v13";
import ms from "ms";
        ...
        process.once("message", async (message: any) => {
            if (message.code === "roleAdded") {
                ...
                if (model.requireStream) {
                    const udp: any = await Promise.race([streamer.createStream({ fps: 1, height: 720, width: 1280 }), timeout()]).catch((err) => null);
                    if (!udp) {
                        process.send({
                            code: "error",
                            ...
                            activities: [
                                {
                                    ...
                                    name: appData.name,
                                    platform: message.code === "setRunningGamePs5" ? "ps5" : message.code === "setRunningGameXbox" ? "xbox" : "desktop",
